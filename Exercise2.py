from lab4_robotics2 import * # Includes numpy import
...
# # Task hierarchy definition for Exercise 1
# tasks = [ 
...
# Task hierarchy definition for Exercise 2
tasks = [ 
            Position2D("End-effector position", np.array([1.0, 0.5]).reshape(2, 1), 3),
            Orientation2D("Link 2 Orientation", np.array([[0.0]]),2)
        ] 
...
def init():
    ...
    tasks[0].setDesired(np.random.uniform(-1.2, 1.2, (2, 1)))
...
# Run simulation with different K values for the first task (for Exercise 2)
# K = np.eye(2)
# K = np.eye(2) * 2
K = np.eye(2) * 5

# Set K matrix for the first task
tasks[0].setGainMatrix(K)
# Run simulation
